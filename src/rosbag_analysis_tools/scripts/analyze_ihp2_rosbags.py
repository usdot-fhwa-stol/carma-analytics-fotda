...
# Helper function: Generates a plot with speed profiles of two platooning vehicles: one follower vehicle and one leader vehicle
def generate_two_vehicle_speed_plot(leader_vehicle_rosbag, follower_vehicle_rosbag, time_starting_vehicle_start_engagement, time_starting_vehicle_end_engagement, veh_1_speed_limit_zone_change_times, first_joiner_platoon_start_time, test_type, test_num, joiner_vehicle_num):

    # Get the true vehicle speed (m/s) and the associated time with each data point
    first = True
    leader_vehicle_speed_times = []
    leader_vehicle_speeds = []
    for topic, msg, t in leader_vehicle_rosbag.read_messages(topics=['/hardware_interface/vehicle/twist'], start_time = time_starting_vehicle_start_engagement, end_time = time_starting_vehicle_end_engagement):
        if first:
            time_start = t
            first = False
            continue

        leader_vehicle_speed_times.append((t-time_start).to_sec())
        leader_vehicle_speeds.append(msg.twist.linear.x) # Current speed in m/s

    follower_vehicle_speed_times = []
    follower_vehicle_speeds = []
    for topic, msg, t in follower_vehicle_rosbag.read_messages(topics=['/hardware_interface/vehicle/twist'], start_time = time_starting_vehicle_start_engagement, end_time = time_starting_vehicle_end_engagement):
        follower_vehicle_speed_times.append((t-time_start).to_sec())
        follower_vehicle_speeds.append(msg.twist.linear.x) # Current speed in m/s

    # Create the initial plot with the defined figure size
    fig, ax = plt.subplots(figsize=(9,5.5))

    # Plot leader and follower vehicle speed (m/s) vs. time
    if test_type == "SLR" or test_type == "ALR" or test_type == "SH-MID" or test_type == "SH-HIGH":
        ax.plot(leader_vehicle_speed_times, leader_vehicle_speeds, 'g:', label='Vehicle 1 Speed (m/s)')
        ax.plot(follower_vehicle_speed_times, follower_vehicle_speeds, 'b:', label="Vehicle " + str(joiner_vehicle_num) + " Speed (m/s)")
    elif test_type == "SLF" or test_type == "ALF":
        if joiner_vehicle_num == 2:
            ax.plot(leader_vehicle_speed_times, leader_vehicle_speeds, 'g:', label='Vehicle 1 Speed (m/s)')
            ax.plot(follower_vehicle_speed_times, follower_vehicle_speeds, 'b:', label="Vehicle " + str(joiner_vehicle_num) + " Speed (m/s)")
        elif joiner_vehicle_num == 3:
            ax.plot(leader_vehicle_speed_times, leader_vehicle_speeds, 'g:', label='Vehicle 2 Speed (m/s)')
            ax.plot(follower_vehicle_speed_times, follower_vehicle_speeds, 'b:', label="Vehicle " + str(joiner_vehicle_num) + " Speed (m/s)")

    # Plot vertical bar at time that first follower joins platoon
    if test_type == "SLR" or test_type == "ALR":
        ax.axvline(x = (first_joiner_platoon_start_time - time_start).to_sec(), color = 'b', label = "Time Vehicle " + str(joiner_vehicle_num) + " Joins Platoon")
    elif test_type == "SLF" or test_type == "ALF":
        if joiner_vehicle_num == 3:
            ax.axvline(x = (first_joiner_platoon_start_time - time_start).to_sec(), color = 'b', label = "Time Vehicle " + str(joiner_vehicle_num) + " Joins Platoon")
        elif joiner_vehicle_num == 2:
            ax.axvline(x = (first_joiner_platoon_start_time - time_start).to_sec(), color = 'g', label = "Time Vehicle 1 Joins Platoon")
    ...
    ax.set_title(str(test_type) + " Test Case Run " + str(test_num) + " Vehicle Speeds -- Vehicle " + str(joiner_vehicle_num)) # Plot Title
    ...
    filename = str(test_type) + "_Run_" + str(test_num) + "_2Vehicle_Speeds_Follower_Vehicle" + str(joiner_vehicle_num) + ".png"
...
# Helper function: Generates a plot with speed profiles of three platooning vehicles: two follower vehicles and one leader vehicle
def generate_three_vehicle_speed_plot(vehicle_1_rosbag, vehicle_2_rosbag, vehicle_3_rosbag, time_starting_vehicle_start_engagement, time_starting_vehicle_end_engagement, first_joiner_platoon_start_time, second_joiner_platoon_start_time, test_type, test_num):
    # Create the initial plot with the defined figure size
    fig, ax = plt.subplots(figsize=(9,5.5))
    ...
    if test_type == "SLR" or test_type == "ALR":
        ...
        # Plot vertical bar at time that first follower joins platoon
        ax.axvline(x = (first_joiner_platoon_start_time - time_start).to_sec(), color = 'b', label = 'Time Vehicle 2 Joins Platoon')

        # Plot vertical bar at time that second follower joins platoon
        ax.axvline(x = (second_joiner_platoon_start_time - time_start).to_sec(), color = 'r', label = 'Time Vehicle 3 Joins Platoon')
        ...
        # Plot vertical bar at time that first follower joins platoon
        ax.axvline(x = (first_joiner_platoon_start_time - time_start).to_sec(), color = 'r', label = 'Time Vehicle 3 Joins Platoon')

        # Plot vertical bar at time that second follower joins platoon
        ax.axvline(x = (second_joiner_platoon_start_time - time_start).to_sec(), color = 'g', label = 'Time Vehicle 1 Joins Platoon')
    ...
    # Plot vehicle 3 speed (m/s) vs. time
    ...
    # Option 1: Save the plot
    filename = str(test_type) + "_Run_" + str(test_num) + "_3Vehicle_Speeds.png"
    plt.savefig(filename, bbox_inches='tight')
    plt.close()
...
# Helper function: Generates a plot with speed profile of an individual vehicle
...
# Helper function: Generates a plot for a follower vehicle, which includes its desired gap and actual gap with the dynamic leader
def generate_platooning_plot(bag, time_start_platooning, bag_file_name, test_type, test_num, vehicle_number):
    # num = 0
    # for topic, msg, t in bag.read_messages(topics=['/guidance/platooning_info'], start_time = time_start_platooning + rospy.Duration(50.0), end_time = time_start_platooning + rospy.Duration(55.0)):
    #     print("*******")
    #     print("host_cmd_speed: " + str(msg.host_cmd_speed))
    #     print("actual_gap: " + str(msg.actual_gap))
    #     num += 1
    #     if num == 10:
    #         return
    actual_gaps = []
    ...
    if test_type == "SLR" or test_type == "SH-MID" or test_type == "SH-HIGH":
        if vehicle_number == 2:
            plt.ylim(0, 40)
        elif vehicle_number == 3:
            plt.ylim(0,50)
    elif test_type == "SLF" or test_type == "ALR" or test_type == "ALF":
        if vehicle_number == 3:
            plt.ylim(0,60)
    ...
    plt.rc('legend', fontsize=10)  # fontsize of the legend text
    ax.legend(loc = 'lower right') # Location of the legend
    ax.set_title(str(test_type) + " Test Case Run " + str(test_num) + " Platooning Information -- Vehicle " + str(vehicle_number)) # Plot Title
    ax.set_xlabel("Time (seconds) Since Vehicle " + str(vehicle_number) + " Joined Platoon") # Plot X Title
    ax.set_ylabel("Meters") # Plot Y Title

    # Option 1: Save the plot
    filename = str(test_type) + "_Run_" + str(test_num) + "_Platooning_Information_Vehicle" + str(vehicle_number) + ".png"
    plt.savefig(filename, bbox_inches='tight')
    plt.close()
...
# Helper function: Generates a plot with the crosstrack profile for an individual vehicle
...
# Helper Function: Get start and end times of the period of engagement; assumes there is only one segment of engagement within the rosbag
    ...
    for test_case_files in ihp2_bag_files:
        ...
        for file_number in range(0, len(bag_files_list)):
            ...
            if vehicle_number == 1:
                ...
                # OPTIONAL: Generate speed and crosstrack plots for this individual vehicle:
        ...
        if test_type == "SLR":
            ...
            generate_two_vehicle_speed_plot(vehicle_1_rosbag, vehicle_2_rosbag, time_1_start_engagement, time_1_end_engagement, veh_1_speed_limit_zone_change_times, vehicle_2_platooning_times[1], test_type, test_num, joiner_vehicle_num=2)
            generate_two_vehicle_speed_plot(vehicle_1_rosbag, vehicle_3_rosbag, time_1_start_engagement, time_1_end_engagement, veh_1_speed_limit_zone_change_times, vehicle_3_platooning_times[1], test_type, test_num, joiner_vehicle_num=3)
            ...
            generate_platooning_plot(vehicle_2_rosbag, vehicle_2_platooning_times[1], vehicle_2_rosbag_name, test_type, test_num, 2)
            generate_platooning_plot(vehicle_3_rosbag, vehicle_3_platooning_times[1], vehicle_3_rosbag_name, test_type, test_num, 3)

            generate_three_vehicle_speed_plot(vehicle_1_rosbag, vehicle_2_rosbag, vehicle_3_rosbag, time_1_start_engagement, time_1_end_engagement, vehicle_2_platooning_times[1], vehicle_3_platooning_times[1], test_type, test_num)
        
        elif test_type == "SLF":
            # Vehicle 3 joins Vehicle 2 from rear; then Vehicle 1 joins 2/3 from Front

            vehicle_1_platooning_times = joiner_platoon_formation_analysis(vehicle_1_rosbag, test_type, vehicle_2_type, vehicle_1_type, 1) # [time_received_initial_ack, time_received_second_ack]
            vehicle_3_platooning_times = joiner_platoon_formation_analysis(vehicle_3_rosbag, test_type, vehicle_2_type, vehicle_3_type, 3) # [time_received_initial_ack, time_sent_second_ack]

            generate_two_vehicle_speed_plot(vehicle_2_rosbag, vehicle_3_rosbag, time_2_start_engagement, time_2_end_engagement, veh_1_speed_limit_zone_change_times, vehicle_3_platooning_times[1], test_type, test_num, joiner_vehicle_num=3)
            generate_two_vehicle_speed_plot(vehicle_1_rosbag, vehicle_2_rosbag, time_2_start_engagement, time_2_end_engagement, veh_1_speed_limit_zone_change_times, vehicle_1_platooning_times[1], test_type, test_num, joiner_vehicle_num=2)

            downtrack_distance_analysis(vehicle_1_rosbag, vehicle_2_rosbag, test_type, 1, 2, vehicle_1_platooning_times[0])
            downtrack_distance_analysis(vehicle_3_rosbag, vehicle_2_rosbag, test_type, 3, 2, vehicle_3_platooning_times[0])

            generate_platooning_plot(vehicle_3_rosbag, vehicle_3_platooning_times[1], vehicle_3_rosbag_name, test_type, test_num, vehicle_number=3)
            generate_platooning_plot(vehicle_2_rosbag, vehicle_1_platooning_times[1], vehicle_2_rosbag_name, test_type, test_num, vehicle_number=2)

            generate_three_vehicle_speed_plot(vehicle_1_rosbag, vehicle_2_rosbag, vehicle_3_rosbag, time_2_start_engagement, time_2_end_engagement, vehicle_3_platooning_times[1], vehicle_1_platooning_times[1], test_type, test_num)

        elif test_type == "ALR":
            # Vehicle 2 joins Vehicle 1 from rear; then Vehicle 3 joins 1/2 from adjacent rear

            vehicle_2_platooning_times = joiner_platoon_formation_analysis(vehicle_2_rosbag, test_type, vehicle_1_type, vehicle_2_type, 2) # [time_received_initial_ack, time_received_second_ack]
            vehicle_3_platooning_times = joiner_platoon_formation_analysis(vehicle_3_rosbag, test_type, vehicle_1_type, vehicle_3_type, 3) # [time_received_initial_ack, time_received_second_ack, time_received_third_ack]
            ...
            generate_two_vehicle_speed_plot(vehicle_1_rosbag, vehicle_2_rosbag, time_1_start_engagement, time_1_end_engagement, veh_1_speed_limit_zone_change_times, vehicle_2_platooning_times[1], test_type, test_num, joiner_vehicle_num=2)
            generate_two_vehicle_speed_plot(vehicle_1_rosbag, vehicle_3_rosbag, time_1_start_engagement, time_1_end_engagement, veh_1_speed_limit_zone_change_times, vehicle_3_platooning_times[2], test_type, test_num, joiner_vehicle_num=3)
            
            generate_platooning_plot(vehicle_2_rosbag, vehicle_2_platooning_times[1], vehicle_2_rosbag_name, test_type, test_num, 2)
            generate_platooning_plot(vehicle_3_rosbag, vehicle_3_platooning_times[2], vehicle_3_rosbag_name, test_type, test_num, 3)

            generate_three_vehicle_speed_plot(vehicle_1_rosbag, vehicle_2_rosbag, vehicle_3_rosbag, time_1_start_engagement, time_1_end_engagement, vehicle_2_platooning_times[1], vehicle_3_platooning_times[2], test_type, test_num)
        
        elif test_type == "ALF":
            # Vehicle 3 joins Vehicle 2 from rear; then Vehicle 1 joins 2/3 from adjacent front

            vehicle_3_platooning_times = joiner_platoon_formation_analysis(vehicle_3_rosbag, test_type, vehicle_2_type, vehicle_3_type, 3) # [time_received_initial_ack, time_received_second_ack]
            vehicle_1_platooning_times = joiner_platoon_formation_analysis(vehicle_1_rosbag, test_type, vehicle_2_type, vehicle_1_type, 1) # [time_received_initial_ack, time_received_second_ack, time_sent_third_ack]
            ...
            generate_two_vehicle_speed_plot(vehicle_2_rosbag, vehicle_3_rosbag, time_2_start_engagement, time_2_end_engagement, veh_1_speed_limit_zone_change_times, vehicle_3_platooning_times[1], test_type, test_num, joiner_vehicle_num=3)
            generate_two_vehicle_speed_plot(vehicle_1_rosbag, vehicle_2_rosbag, time_2_start_engagement, time_2_end_engagement, veh_1_speed_limit_zone_change_times, vehicle_1_platooning_times[2], test_type, test_num, joiner_vehicle_num=2)

            generate_platooning_plot(vehicle_3_rosbag, vehicle_3_platooning_times[1], vehicle_3_rosbag_name, test_type, test_num, vehicle_number=3)
            generate_platooning_plot(vehicle_2_rosbag, vehicle_1_platooning_times[2], vehicle_2_rosbag_name, test_type, test_num, vehicle_number=2)

            generate_three_vehicle_speed_plot(vehicle_1_rosbag, vehicle_2_rosbag, vehicle_3_rosbag, time_2_start_engagement, time_2_end_engagement, vehicle_3_platooning_times[1], vehicle_1_platooning_times[2], test_type, test_num)
            ...
            generate_platooning_plot(vehicle_2_rosbag, vehicle_2_platooning_times[1], vehicle_2_rosbag_name, test_type, test_num, vehicle_number=2)
            generate_two_vehicle_speed_plot(vehicle_1_rosbag, vehicle_2_rosbag, time_1_start_engagement, time_1_end_engagement, veh_1_speed_limit_zone_change_times, vehicle_2_platooning_times[1], test_type, test_num, joiner_vehicle_num=2)
