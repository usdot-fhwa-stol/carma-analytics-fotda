...
class CDA1tenthMetricAnalysis(unittest.TestCase):
    ...
    def test_CDA1tenth_01_generates_route_from_rviz_timing(self):
        # The CDA1tenth CMV will generate a route within 3 seconds of receiving a goal pose from Rviz
    ...
    def test_CDA1tenth_02_generates_route_from_MOM(self):
        # The CDA1tenth CMV is able to receive destinations as Mobility Operation messages from the infrastructure and generate a route
        self.assertTrue(check_message_published(self.bag_dir, "/plan"), "Message was not received on /plan")

    def test_CDA1tenth_03_follows_road_network(self):
        # The CDA1tenth CMV will drive along the centerline of the defined road network with less than 0.2 m of crosstrack error 95% of the time
    ...
    def test_CDA1tenth_04_follows_road_network(self):
        # The CDA1tenth CMV will drive along the centerline of the defined road network with less than 0.3 m of crosstrack error
        _, crosstrack_errors = plot_crosstrack_error(self.bag_dir, "/route_graph", show_plots=False)
        self.assertTrue(np.all(np.abs(crosstrack_errors) < 0.3), "Crosstrack error from road network exceeded 0.2 m")

    def test_CDA1tenth_05_maintains_speed_on_straights(self):
        # The CDA1tenth CMV achieves and maintains its target speed with a tolerance of 0.2 m/s (excluding turns)
    ...
    def test_CDA1tenth_06_slowdown_on_turns(self):
        # The CDA1tenth CMV will not slowdown less than 50% of the target speed during turns
        velocities, target_velocities = plot_vehicle_speed(self.bag_dir, show_plots=False)
        max_target_speed = np.max(target_velocities)
    ...
    def test_CDA1tenth_07_stops_close_to_destination(self):
        # The CDA1tenth CMV stops within 0.5 m of the goal destinations
        arrival_distances = check_distance_to_arrival(self.bag_dir)
        print(arrival_distances)
        self.assertTrue(np.all(arrival_distances < 0.5), "Vehicle did not stop within 0.5 m of goal destination")

    def test_CDA1tenth_08_acks_with_MOM(self):
        # The CDA1tenth CMV sends Mobility Operation message acknowledging when a goal is reached
    ...
    def test_CDA1tenth_09_receives_incoming_MOM(self):
        # The CDA1tenth CMV sends Mobility Operation message acknowledging when a goal is reached
    ...
    def test_CDA1tenth_10_generates_route_from_MOM_timing(self):
        # The CDA1tenth CMV will generate a route within 3 seconds of receiving an incoming mobility operation message
    ...
    def test_CDA1tenth_13_communicate_load_and_unload(self):
        # Infrastructure can communicate with the CDA1tenth CMV that the container is loaded and unloaded
    ...
    def test_CDA1tenth_14_communicate_checkpoint(self):
        # Infrastructure can communicate with the CDA1tenth CMV that the container is loaded and unloaded
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run CDA1tenth metric analysis")
