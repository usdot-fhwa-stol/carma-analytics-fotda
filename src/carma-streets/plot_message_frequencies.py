...
def plot_message_frequencies(csv_dir: Path, plots_dir: Path, simulation: bool = False):
    ...
    if csv_dir.is_dir():
        ...
        for msg_plot,( message_name, message_data_frame) in zip( plots, message_data.items()):
            ...
            # print(min(message_data_frame["Average Frequency (Hz)"]), max(message_data_frame["Average Frequency (Hz)"]))
        # print('\n')
...
def plot_message_frequency( axes: axes.Axes, time: list, frequency: list , message_name: str, target_frequency: float = 10.0, absolute_error: float = 2.0) :
    ...
    """
    # print(message_name)
    # if (message_name != 'sdsm'):
    cleanedList = [x for x in frequency if str(x) != 'nan']
    print(min(cleanedList), max(cleanedList))
    # elif (message_name == 'sdsm'):
    #     print('\n')
