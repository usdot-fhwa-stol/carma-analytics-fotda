    ...
    for odom in odometry:
        ...
        for node in nx_graph.nodes(data=True):
            if np.linalg.norm(node[1]['pos'] - odom) < min_distance:
                ...
                min_node = node
        # Add node if it is the first node
        if len(route_coordinates_reached) == 0:
            ...
            # If the node has multiple branches, mark the next two nodes on each branch as "blocked", meaning 
            # we won't add them to the list of visited nodes when we reach them. The intent here is to wait until 
            # the vehicle traverses a branch further to figure out which path was taken and then work backwards to
            # determine the path it took
            ...
            if len(blocked_nodes) == 0 and min_node[0] in nx_graph.neighbors(previous_node_id):
                ...
                if len(list(nx_graph.neighbors(min_node[0]))) > 1:
                    ...
                    for neighbor in nx_graph.neighbors(min_node[0]):
                        ...
                        for next_neighbor in nx_graph.neighbors(neighbor):
                            blocked_nodes.add(next_neighbor)
            # If we have nodes marked as blocked (meaning we are taking a branch) but the closest node is
            # not in the set of blocked nodes, find out which path we took from the node that is the root
            # of the branch and mark these nodes as visited
