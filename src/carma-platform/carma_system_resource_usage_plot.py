...
def plot_system_usage(csv_file, notable_event_stamps=[], cpu_number=DEFAULT_CPU_NUM, output_file=None):
    """
    ...
    try:
        ...
        ax1.plot(timestamps, cpu_values,
                label='Average CPU per Core (%)',
                color='blue',
                linewidth=2)
        ax1.plot(timestamps, total_cpu_values,
                label='Total CPU (%)',
                color='red',
                linewidth=2)
        
        # Plot Memory usage (bottom subplot)
        ax2.plot(timestamps, memory_values,
        ...
        if notable_event_stamps:
            ...
            for event_name, event_time in notable_event_stamps:
                if event_time >= timestamps[0] and event_time <= timestamps[-1]:
                    ...
                    ax1.text(event_time, cpu_max * 0.95, event_name,
                            rotation=90, verticalalignment='top', horizontalalignment='right')
                    ax2.text(event_time, mem_max * 0.95, event_name,
                            rotation=90, verticalalignment='top', horizontalalignment='right')
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Generate CPU and Memory usage graph from CSV data.')
