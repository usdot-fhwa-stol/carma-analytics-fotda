...
do
    ...
    then
        ...
        then
            ...
            aws s3 cp $FILENAME s3://$bucketname/$S3_FILENAME --region $region --profile saml
            ENDTIME=$(date +"%s.%N")
            ...
            do
                ...
                then
                    if [ "$input_1" = "y"  ]
                    then
                        aws s3 rm s3://$bucketname/$S3_FILENAME --region $region --profile saml
                        j=$(( j-1 ))
                    else
                        j=$(( j-1 ))
                    fi
                else
                    echo "1. Invalid input. Valid inputs are 'y' or 'n'."
                fi
            done
        ...
        ## Uploading the file to the S3 Bucket via a Transfer Acceleration.
            ...
            aws s3 cp $FILENAME s3://$bucketname/$S3_FILENAME --region $region --endpoint-url http://s3-accelerate.amazonaws.com --profile saml
            ENDTIME=$(date +"%s.%N")
            ENDTIME_us=$(expr $ENDTIME*$((1000000)) | bc -l)
            Time_elapsed=$(expr $ENDTIME_us-$STARTTIME_us | bc -l)
            Throughput=$(expr $(($FILESIZE*8))/$Time_elapsed | bc -l)
            ...
            do
                ...
                then
                    ...
                    echo "2. Invalid input. Valid inputs are 'y' or 'n'."
                fi
            done
        fi
    else
        echo "3. Invalid input. Valid inputs are 'y' or 'n'."
    fi
done
